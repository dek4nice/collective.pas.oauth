...
class GithubLoginView(OAuthLogin):
    """Github OAuth 2.0 login view"""
    ...
    def __call__(self):
        ...
        if errorReason is not None:
            IStatusMessage(self.request).add(_(u"Github authentication denied"), type="error")
        ...
        if not userId or not userFullname:
            IStatusMessage(self.request).add(_(u"Insufficient information in Github profile"), type="error")
            redirect(self.context.absolute_url())
            return u""
